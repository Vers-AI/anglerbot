...
from ares.behaviors.combat.individual import  StutterUnitBack, KeepUnitSafe, PathUnitToTarget
...
class AnglerBot(AresBot):
    ...
    async def on_step(self, iteration: int):
        ...
        if not self.enemy_pylon or not self.pylon:
        ...
        if self.enemy_units: # used to track enemies for final attack
            self.enemy_supply = self.get_total_supply(self.enemy_units)
        ...
        ## Chooses the Target for Control_Attackers
        ...
        ## Scout controls
        if self.map == "BMA" and self.launch_late_attack:
    ...
    def control_attackers(self, attackers: Units) -> None:
        ...
        unit_scores = self.calculate_scores(self.enemy_units)
        ...
        for squad in squads:
            ...
            if ranged:
                # Moving Stalkers to Initial Positions
                ...
                # Attack enemies in range
                    ...
                    for unit in ranged:
                        ...
                        target_unit = sorted(self.enemy_units, key=lambda x: unit_scores[x.tag] - (x.distance_to(unit.position) * x.distance_to(unit.position)), reverse=True)[0]
